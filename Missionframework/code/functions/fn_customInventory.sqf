...
if (_type isEqualTo 1) then {
	...
	_maxBackpacks = round (_cfgTransportMaxBackpacks * 0.1);
	_maxMagazines = round (_cfgTransportMaxMagazines * 0.7);
	_maxWeapons = round (_cfgTransportMaxWeapons * 0.15);
	_qtyBackpacks = round (_maxBackpacks / 2);
	_qtyMagazines = round (_maxMagazines / 1);
	_qtyWeapons = round (_maxWeapons / 2);
	...
	if ((_entityType in [
		...
		"b_cargonet_01_ammo_f","i_cargonet_01_ammo_f","o_cargonet_01_ammo_f","i_e_cargonet_01_ammo_f","i_eaf_supplycrate_f"
	]) || (_entity isKindOf 'LandVehicle')) then {
		if (_entityType in ["b_cargonet_01_ammo_f","i_cargonet_01_ammo_f","o_cargonet_01_ammo_f",'i_e_cargonet_01_ammo_f']) then {
		...
		//comment 'Items';
		{
			...
			['FirstAidKit',(round (20 + (random 15)))],
			['SmokeShell',(round (5 + (random 5)))],
			['SmokeShellBlue',(round (2 + (random 2)))],
			['SmokeShellRed',(round (2 + (random 2)))],
			['1Rnd_HE_Grenade_shell',(round (6 + (random 12)))],
		...
		for '_x' from 0 to (_maxMagazines - 1) step 1 do {
			_magazineType = selectRandomWeighted [
				'30Rnd_556x45_Stanag_Tracer_Red',0.8,
				'30Rnd_65x39_caseless_mag_Tracer',0.8,
				'30Rnd_65x39_caseless_msbs_mag',0.8,
				'30Rnd_762x39_AK12_Mag_F',0.4,
				'20Rnd_762x51_Mag',0.4,
				'200Rnd_556x45_Box_F',0.2,
				'150Rnd_556x45_Drum_Mag_F',0.2,
				'150Rnd_762x54_Box',0.2,
				'200Rnd_65x39_cased_Box',0.2,
				'150Rnd_93x64_Mag',0.1,
				'130Rnd_338_Mag',0.1,
				'75rnd_762x39_AK12_Mag_F',0.1,
				'MRAWS_HEAT_F',0.2,
				'Vorona_HEAT',0.1,
				'Titan_AT',0.1,
				'NLAW_F',0.1
