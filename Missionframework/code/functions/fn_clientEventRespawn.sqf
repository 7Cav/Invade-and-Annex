...
if (([] call (missionNamespace getVariable 'QS_fnc_clientGetSupporterLevel')) > 0) then {
	0 spawn {
		...
		if (!((player getVariable 'QS_ClientBTexture2') isEqualTo '')) then {
			if (!((player getVariable 'QS_ClientBTexture2_Backpacks2') isEqualTo [])) then {
				if ((backpack player) in (player getVariable 'QS_ClientBTexture2_Backpacks2')) then {
					unitBackpack player setObjectTextureGlobal [0, (player getVariable 'QS_ClientBTexture2')];
				};
			};
		};
	};
...
if ((player getVariable 'QS_revive_respawnType') in ['BASE','']) then {
	if (((player getUnitTrait 'QS_trait_fighterPilot') || (player getUnitTrait 'QS_trait_pilot')) && (player getVariable 'QS_revive_respawnType') isEqualTo 'BASE') then {
		private _alivePilots = allPlayers select {((_x getUnitTrait 'QS_trait_fighterPilot') || (_x getUnitTrait 'QS_trait_pilot')) && lifeState _x in ["HEALTHY", "INJURED"] && player != _x};
		private _numBasePilots = {(_x distance2D (markerPos 'QS_marker_base_marker')) < 1000} count _alivePilots;
		private _aliveStr = if(player getVariable ["McD_RespawnWatch_alive", FALSE]) then {"alive"} else {"dead"};
		private _respawnPos = player getVariable ["McD_RespawnWatch_RespawnPosition", [0,0,0]];
		private _msg = format ["Pilot %1 (%4 at grid %5) respawned back to base. Number of alive pilots at the time: %2, %3 of them at base.", profileName, count _alivePilots, _numBasePilots, _aliveStr, mapGridPosition _respawnPos];
		if(count _alivePilots >= 2) then {
			if(_numBasePilots >= 2) then {
				_msg = _msg + " With those numbers, a valid reason to respawn is rather unlikely. Did the pilot really follow the handbook?";
			} else {
				_msg = _msg + " Please make sure to follow the pilot handbook.";
			};
		} else {
			if(count _alivePilots == 0) then {
				_msg = format ["No pilots were alive, so pilot %1 (%2 at grid %3) respawned back to base", profileName, _aliveStr, mapGridPosition _respawnPos];
			};
		};
		['sideChat',[WEST,'PAPA_BEAR'], _msg] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
	};
