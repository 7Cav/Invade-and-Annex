...
if (_state isEqualTo 1) then {
	...
	if (_aoPos inPolygon [
		...
		[0,"Medevac wounded soldier",0,70] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		[
			'QS_IA_TASK_AO_3',
			TRUE,
			[
				'A recon soldier has been wounded in the AO and requires urgent medevac. Get him back to the Medevac HQ at base (small white building) to complete the mission. If he dies or bleeds out (about 60 minutes from mission start), the mission is failed. You need to do this mission to win the AO. Good luck soldiers!',
				'Medevac',
				'Medevac'
			],
			[_unit,TRUE],
...
if (_state isEqualTo 2) then {
	...
	if (serverTime > _missionDuration) exitWith {
		...
		[2,"Medevac failed, soldier bled out",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if ((((_unit distance _missionDestination) < 5) && (isNull (attachedTo _unit))) || (([0,_unit] call (missionNamespace getVariable 'QS_fnc_isNearFieldHospital')) && (isNull (attachedTo _unit)) && (isNull (objectParent _unit)))) exitWith {
		//comment 'Mission success';
		//['ST_MEDEVAC',['Medevac','Medevac complete!']] remoteExec ['QS_fnc_showNotification',-2,FALSE];
		[1,"Medevac complete",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if (!alive _unit) exitWith {
		...
		[2,"Medevac failed, soldier killed",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		['QS_IA_TASK_AO_3'] call (missionNamespace getVariable 'BIS_fnc_deleteTask');
		_return = [
			_case,
			0,
