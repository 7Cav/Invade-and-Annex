...
private [
	'_type','_uid','_moderators','_admins','_developers','_curators','_allUIDs','_moderatorsActions','_adminsActions',
	'_developersActions','_curatorsActions','_validActions','_text','_pilots','_cursorTarget','_type2','_staffID'
];
...
if ((_type2 isEqualType '') && (_type2 isEqualTo 'KeyDown')) exitWith {
	...
	_moderatorsActions = [
		...
		['1 - Show Pilots',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),6,80,TRUE,TRUE,'','TRUE'],
		['2 - Command Channel (Toggle)',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),10,79,TRUE,TRUE,'','TRUE'],
		['3 - Aircraft Channel (Toggle)',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),11,78,TRUE,TRUE,'','TRUE'],
		['4 - Spectate',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),4,77,TRUE,TRUE,'','TRUE']
	];
	_adminsActions = [
	//['6 - (Target) Pardon',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),8,76,TRUE,TRUE,'','TRUE'],
	//['7 - (Target) Punish',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),9,76,TRUE,TRUE,'','TRUE'],
		['5 - (Target) Delete',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),3,75,TRUE,TRUE,'','TRUE'],
		['6 - (Target) Repair',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),7,74,TRUE,TRUE,'','TRUE'],
		['7 - (Target) Revive',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),12,73,TRUE,TRUE,'','TRUE'],
		['8 - Invincibility',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),5,72,TRUE,TRUE,'','TRUE'],
		['9 - Map Teleport',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),13,71,TRUE,TRUE,'','TRUE']
	];
	_developersActions = [
		['10 - (Clean) Base',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),14,70,TRUE,TRUE,'','TRUE'],
		['11 - (Clean) Island',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),15,69,TRUE,TRUE,'','TRUE'],
		['12 - Splendid Cam',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),16,68,TRUE,TRUE,'','TRUE'],
		['13 - Dev Terminal',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),17,67,TRUE,TRUE,'','TRUE']
	];
	_mediaActions = [
		['12 - Splendid Cam',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),16,68,TRUE,TRUE,'','TRUE']
	];
	_curatorsActions = [
		['14 - (Target) Disable Gear Restrictions',(missionNamespace getVariable 'QS_fnc_clientMenuStaff'),18,76,TRUE,TRUE,'','TRUE']
	];
...
if (_type2 isEqualType 0) exitWith {
	...
	if (_type2 isEqualTo 4) then {
		...
		if (!isStreamFriendlyUIEnabled) then {
			(missionNamespace getVariable 'QS_managed_hints') pushBack [5,FALSE,5,-1,'[Alt]+[F4] to Exit spectator mode.... Just kidding [Shift]+[F1]',[],(serverTime + 10)];
		};
	};
	...
	if (_type2 isEqualTo 10) then {
		McD_adminForcedRadioChannels set [0, !(McD_adminForcedRadioChannels # 0)];
	};
	if (_type2 isEqualTo 11) then {
		McD_adminForcedRadioChannels set [1, !(McD_adminForcedRadioChannels # 1)];
	};
	...
	if (_type2 isEqualTo 18) then {
		...
		_cursorTarget setVariable ["McD_GearRestrictionDisabled", TRUE, TRUE];
		_text = format ['%1 (staff) allows %2 to bypass gear restrictions',profileName,(name _cursorTarget)];
		['systemChat',_text] remoteExec ['QS_fnc_remoteExecCmd',-2,FALSE];
	};
	{
		player removeAction _x;
