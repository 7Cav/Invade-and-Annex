...
if (_QS_playerCount > 10) then {
	_QS_inBuildingCount = 6;
	_QS_garrisonCount = 14;
	_QS_patrolCount = 4;
	_QS_civilianCount = 7;
};
if (_QS_playerCount > 20) then {
	_QS_inBuildingCount = 8;
	_QS_garrisonCount = 18;
	_QS_patrolCount = 5;
	_QS_civilianCount = 8;
};
if (_QS_playerCount > 30) then {
	_QS_inBuildingCount = 10;
	_QS_garrisonCount = 20;
	_QS_patrolCount = 6;
	_QS_civilianCount = 9;
};
if (_QS_playerCount > 40) then {
	_QS_inBuildingCount = 12;
	_QS_garrisonCount = 22;
	_QS_patrolCount = 9;
...
[6,"Secure Weapons Caches",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
_box1_secured = FALSE;
_box2_secured = FALSE;
_box3_secured = FALSE;
...
for '_x' from 0 to 1 step 0 do {
	...
	if ((_QS_objectArray findIf {!(alive _x)}) > -1) then {
		_QS_missionSuccess = FALSE;
		_QS_missionFailed = TRUE;
	};
	if ((_QS_targetBuildings findIf {!(alive _x) && !((_QS_objectArray select (_QS_targetBuildings find _x)) getVariable ['QS_secured',FALSE]) }) > -1) then {
		_QS_missionSuccess = FALSE;
		_QS_missionFailed = TRUE;
	};
	_QS_allSecured = TRUE;
	...
	if (!(_QS_enemyDetected)) then {
		...
		if (_QS_enemyDetected) then {
			...
			[4,"You have been detected",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
			{
				if (alive _x) then {
		...
		if (!isNull _QS_object1) then {
			if (alive _QS_object1) then {
				if (_QS_object1 getVariable ['QS_secured',FALSE]) then {
					...
					[4,(format ['%1 / 3 intel secured',(missionNamespace getVariable 'QS_mission_urban_objectsSecured')]),0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
				};
			};
		};
	};
