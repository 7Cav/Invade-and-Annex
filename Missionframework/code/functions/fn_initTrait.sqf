...
if (_trait isEqualTo 'uavhacker') then {
	if (_traitValue) then {
		...
		0 spawn {
			...
			waitUntil{!((group player) isEqualTo _oldGroup)};
			["SendClientMessage", ["RegisterGroup", [group player, player, [nil, "UAV", TRUE]]]] call BIS_fnc_dynamicGroups;
		};
		/*
...
if (_trait isEqualTo 'QS_trait_HQ') then {
	if (_traitValue) then {
		...
		[player, "LOGIN"] remoteExec ["NL_fnc_hqTimer", 2, FALSE];
		0 spawn {
			private _oldGroup = group player;
			["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call BIS_fnc_dynamicGroups;
			waitUntil{!((group player) isEqualTo _oldGroup)};
			["SendClientMessage", ["RegisterGroup", [group player, player, [nil, "Commander", TRUE]]]] call BIS_fnc_dynamicGroups;
		};
		/*
		if (!(2 in (missionNamespace getVariable 'QS_client_radioChannels'))) then {
			[1,2] call (missionNamespace getVariable 'QS_fnc_clientRadio');
		...
		*/
		...
		[player, "LOGOUT"] remoteExec ["NL_fnc_hqTimer", 2, FALSE];
		["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call BIS_fnc_dynamicGroups;
		/*
		if (!(_role in ['pilot','pilot_heli','pilot_plane','uav','pilot_cas','commander','jtac'])) then {
			[0,2] call (missionNamespace getVariable 'QS_fnc_clientRadio');
...
if (_trait isEqualTo 'QS_trait_pilot') then {
	if (_traitValue) then {
		0 spawn {
			...
			["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call BIS_fnc_dynamicGroups;
			waitUntil{!((group player) isEqualTo _oldGroup)};
			["SendClientMessage", ["RegisterGroup", [group player, player, [nil, format ["Pilot %1", profileName], TRUE]]]] call BIS_fnc_dynamicGroups;
		};
		/*
		if (!(2 in (missionNamespace getVariable 'QS_client_radioChannels'))) then {
			[1,2] call (missionNamespace getVariable 'QS_fnc_clientRadio');
