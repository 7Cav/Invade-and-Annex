...
if (_type isEqualTo 'BRIEF') then {
	...
	missionNamespace setVariable ['NL_mainMissionActive', TRUE, FALSE];
	{
	...
	if (!(missionNamespace getVariable 'QS_mainao_firstRun')) then {
		...
		[0,"Take the AO",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	} else {
	...
	};
	{
		...
		[
			...
			TRUE,
			[
				'Seize the enemy headquarters to degrade enemy coordination. This objective is completed when the enemy commander dies. He is usually located in the immediate vicinity of the HQ. This objective is  a requirement in order to finish the mission.',
				'Enemy HQ',
				'Seize Enemy HQ'
			],
		...
		[
			...
			TRUE,
			[
				'Destroy the radio tower. Destroying the radio tower will degrade enemy communications and prevent them from calling in air support. This objective is not accurately marked. The radio tower is somewhere within the surrounding circle. This objective is  a requirement in order to finish the mission.',
				'Destroy radiotower',
				'Destroy radiotower'
			],
		...
		[
			...
			TRUE,
			[
				'Clear the marked area of resistance. Once there are under 10 enemies in this area and no more sub-objectives to do, this task will complete (this included Medevac and HVT missions!). You may have to conduct a number of patrols around the zone to finish them off.',
				(format ['Take %1',_ao]),
				(format ['Take %1',_ao])
			],
			_QS_AOpos,
...
if (_type isEqualTo 'DEBRIEF') then {
	missionNamespace setVariable ['NL_mainMissionActive', FALSE, FALSE];
	...
	[1,"The AO has been captured!",0,100] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	[100] call NL_fnc_addScore;
};
