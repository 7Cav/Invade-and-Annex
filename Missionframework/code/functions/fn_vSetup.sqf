...
_fuelCargo = [
	...
	'b_g_van_01_fuel_f','b_truck_01_fuel_f','b_t_truck_01_fuel_f','land_pod_heli_transport_04_fuel_f','land_pod_heli_transport_04_fuel_black_f','o_heli_transport_04_fuel_f',
...
_ammoCargo = [
	...
	'i_truck_02_ammo_f','o_heli_transport_04_ammo_f','land_pod_heli_transport_04_ammo_f','land_pod_heli_transport_04_ammo_black_f','b_slingload_01_ammo_f'
];
_repairCargo = [
	...
	'b_g_offroad_01_repair_f','land_pod_heli_transport_04_repair_f','land_pod_heli_transport_04_repair_black_f','o_truck_03_repair_f','i_truck_02_box_f','o_truck_02_box_f'
];
_gorgon = [
...
/*/============================================= APPLY/*/
if (!isNil { _u getVariable 'NL_vehicle_baseAA' }) then {
	_u lockTurret [[0,0], TRUE];
	_u lockDriver TRUE;
	_u setVariable ["McD_noService", TRUE, TRUE];
	_u setVariable ["McD_rewardRefillsLeft4AO", (missionNamespace getVariable 'McD_missionConfig_baseAA_rewardRefillPerAO'), TRUE];
	_u allowDamage FALSE;
	private _fixPoint = (createGroup [sideLogic,TRUE]) createUnit ['Logic', (position _u) select [0,2] ,[],0,'CAN_COLLIDE'];
	...
	_fixPoint setDir (getDir _u);
	_u attachTo [_fixPoint];
};
...
if (_t in _gryphon) then {
	...
	_u setObjectTextureGlobal [2, "a3\air_f_jets\plane_fighter_04\data\fighter_04_misc_01_co.paa"];
	_u setObjectTextureGlobal [3, "a3\air_f_jets\plane_fighter_04\data\numbers\fighter_04_number_00_ca.paa"];
	_u setObjectTextureGlobal [4, "a3\air_f_jets\plane_fighter_04\data\numbers\fighter_04_number_07_ca.paa"];
	_u setObjectTextureGlobal [5, "a3\air_f_jets\plane_fighter_04\data\numbers\fighter_04_number_07_ca.paa"];
};
...
if (_t in _mohawk) then {
	_u setObjectTextureGlobal [0, "#(argb,8,8,3)color(0.45,0.47,0.39,0.28)"];
	_u setObjectTextureGlobal [1, "#(argb,8,8,3)color(0.45,0.47,0.39,0.28)"];
	_u setObjectTextureGlobal [2, "#(argb,8,8,3)color(0.45,0.47,0.39,0.28)"];
	_u setObjectTextureGlobal [3, "A3\Air_F_Beta\Heli_Transport_02\Data\Heli_Transport_02_int_02_CO.paa"];
};
...
if (!(_isSimpleObject)) then {
	...
	if (!(_z)) then {
		...
		_u setVariable ['QS_disableRespawnAction',TRUE,TRUE];
	...
	if (_u isKindOf 'Helicopter') then {
		...
		if (_t2 in ['b_t_uav_03_f']) then {
			...
			_u addItemCargoGlobal ['SMG_01_Holo_F',1];
			if (local _u) then {
