...
if (isNil {profileNamespace getVariable 'QS_stamina'}) then {
	...
	player setCustomAimCoef (missionNamespace getVariable ['QS_missionConfig_minAimCoef',0.1]);
	...
	if (_qs_stamina isEqualType []) then {
		...
		player setCustomAimCoef (_aimcoef max (missionNamespace getVariable ['QS_missionConfig_minAimCoef',0.1]));
		player setVariable ['QS_stamina',[(isStaminaEnabled player),(getCustomAimCoef player)],FALSE];
	} else {
		if ((missionNamespace getVariable ['QS_missionConfig_stamina',0]) isEqualTo 0) then {
			player enableStamina FALSE;
...
/*/====================== PLAYER OBJECT VARS/*/
{
	...
	["McD_GearRestrictionDisabled", FALSE, FALSE]
];
...
0 spawn {
	...
	if ((getPlayerUID player) in (['S3'] call (missionNamespace getVariable 'QS_fnc_whitelist'))) then {
		...
		// comment 'Backpack';
		if (!isNil {profileNamespace getVariable 'QS_ClientBTexture2'}) then {
			if ((profileNamespace getVariable 'QS_ClientBTexture2') isEqualType '') then {
				if (!((profileNamespace getVariable 'QS_ClientBTexture2') isEqualTo '')) then {
					if (!isNil {profileNamespace getVariable 'QS_ClientBTexture2_Backpacks2'}) then {
						if ((profileNamespace getVariable 'QS_ClientBTexture2_Backpacks2') isEqualType []) then {
							if (!((profileNamespace getVariable 'QS_ClientBTexture2_Backpacks2') isEqualTo [])) then {
								if ((backpack player) in (profileNamespace getVariable 'QS_ClientBTexture2_Backpacks2')) then {
									unitBackpack player setObjectTextureGlobal [0,(profileNamespace getVariable 'QS_ClientBTexture2')];
									player setVariable ['QS_ClientBTexture2',(profileNamespace getVariable 'QS_ClientBTexture2'),FALSE];
									player setVariable ['QS_ClientBTexture2_Backpacks2',(profileNamespace getVariable 'QS_ClientBTexture2_Backpacks2'),FALSE];
								};
							};
						};
					};
...
/*
// grouping you into an open squad
...
/*
// Joining people with the same squad xml squadname together
...
if (isNil {profileNamespace getVariable 'QS_client_radioChannels_profile'}) then {
	...
	if ((count _QS_radioChannels_profile) isEqualTo 10) then {
		...
		if ((_QS_radioChannels_profile select 8) isEqualType TRUE) then {
			if (_QS_radioChannels_profile select 8) then {
				[1,9] call (missionNamespace getVariable 'QS_fnc_clientRadio');
			};
		};
		if ((_QS_radioChannels_profile select 9) isEqualType TRUE) then {
			if (_QS_radioChannels_profile select 9) then {
