...
private [
	...
	'_QS_module_aoSmallTasks_timeout','_QS_module_aoSmallTasks_list','_QS_module_aoSmallTasks_current','_QS_module_aoSmallTasks_data',
...
QS_module_aoSmallTasks_isActive = FALSE;
...
_sideMissionList = [
	...
	[
		0.3,							// Side mission frequency weighting (of the above missions)  ( https://community.bistudio.com/wiki/selectRandomWeighted )
		0.3,
		0.2,
		0.5,
		0.3,
...
_QS_darkAccelerationFactor = 3;
_QS_noonAccelerationFactor = 2;
_QS_dawnDuskDeccelerationFactor = 1;
...
_QS_module_enemyCAS_spawnDelayDefault = 700;
...
_QS_module_restart_script = {diag_log "ERROR: Restart was triggered in core.sqf - It wont be done, but server core encountered a problem";};
//systemTime
...
if (_QS_module_restart) then {
	...
	_QS_module_restart_script = {
		...
		0 = [6,_text,0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_QS_diagTickTimeNow > _fpsCheckDelay) then {
		...
		format [
			...
			while {!(_this isEqualTo "")} do {
				diag_log text (_this select [0, 1023]);
				_this = _this select [1023];
			};
		};
	...
	if (_mainMissions) then {
				...
				if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'CLASSIC') then {
					if (!(_defendAOActive)) then {
						if ((!(missionNamespace getVariable 'QS_aoSuspended')) && (!(missionNamespace getVariable 'QS_customAO_active'))) then {
							if (isNil {_QS_AOpos}) then {
								_QS_AOpos = [-3000, -3000];
							};
							private _aoListNotTooClose = _aoList select { ( _QS_AOpos distance2D (_x # 1) ) > 3000 };
							if (missionNamespace getVariable ['McD_cycleAoRegion', _false]) then {
								_aoListNotTooClose = [];
								missionNamespace setVariable ['McD_cycleAoRegion', _false, _true];
							};
							//if (_aoList isEqualTo []) then {
							if (_aoListNotTooClose isEqualTo []) then {
								_aoList = [];
								...
								_aoListNotTooClose = _aoList select { ( _QS_AOpos distance2D (_x # 1) ) > 3000 };
								if (count _aoListNotTooClose == 0) then {
									_aoListNotTooClose = _aoList;
								};
							};
							//_ao = selectRandom _aoList;
							_ao = selectRandom _aoListNotTooClose;
							diag_log str _ao;
			...
			if (_timeNow > _aoGraceTime) then {
				if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'CLASSIC') then {
					...
					if (_timeNow > _QS_ao_invincibility_checkDelay) then {
						...
						if (!(_minefieldSpawned)) then {
							if (missionNamespace getVariable ['QS_ao_createDelayedMinefield',_false]) then {
								_QS_listNearbyPlayers = [(missionNamespace getVariable 'QS_radioTower_pos'),1000,[_west],(_allPlayers unitsBelowHeight 30),0] call _fn_serverDetector;
								if (!(_QS_listNearbyPlayers isEqualTo [])) then {
					...
					if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'SC') then {
						...
						if (!(_minefieldSpawned)) then {
							if (missionNamespace getVariable ['QS_ao_createDelayedMinefield',_false]) then {
								_QS_listNearbyPlayers = [(missionNamespace getVariable 'QS_radioTower_pos'),1500,[_west],(_allPlayers unitsBelowHeight 30),0] call _fn_serverDetector;
								if (!(_QS_listNearbyPlayers isEqualTo [])) then {
									_minefieldSpawned = _true;
						...
						if (!((missionNamespace getVariable 'QS_virtualSectors_aoMortars') isEqualTo [])) then {
							if (((missionNamespace getVariable 'QS_virtualSectors_aoMortars') select {((alive _x) && (alive (gunner _x)))}) isEqualTo []) then {
								...
								[1,"Mortar pit neutralized",0,20] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		...
		if (_QS_module_aoSmallTasks && _mainMissionActive) then {
			if ((_timeNow > _QS_module_aoSmallTasks_checkDelay) || {(!isNil 'QS_aoCycleSmallTask')}) then {
				if ((_timeNow > 120) || {(!isNil 'QS_aoCycleSmallTask')}) then {
					if (!(QS_module_aoSmallTasks_isActive)) then {
						if ((!(_defendAOActive)) || {(!isNil 'QS_aoCycleSmallTask')}) then {
							if (!(missionNamespace getVariable 'QS_customAO_active')) then {
								if ((_timeNow > _QS_module_aoSmallTasks_timeoutDelay) || {(!isNil 'QS_aoCycleSmallTask')}) then {
									...
									QS_module_aoSmallTasks_isActive = _true;
									...
									if (_QS_module_aoSmallTasks_data isEqualType 0) then {
										QS_module_aoSmallTasks_isActive = _false;
		...
		if (_timeNow > _module_fob_checkDelay) then {
			if (!((missionNamespace getVariable 'QS_activeRegion') isEqualTo _module_fob_activeRegion)) then {
				...
				if (!(_module_fob_activeRegion isEqualTo -1)) then {
					...
					[0,(format ['Secure FOB %1',(([_module_fob_activeRegion] call (missionNamespace getVariable 'QS_data_fobs')) # 5)]),0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
				};
			} else {
				...
				/*
				// FOBs activate vehicles Respawn by default, because the building is a "cargo"-hq-container...
							...
							if ((getMass _x) > 5000) then {
								if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
									if (!(_module_fob_logistics_vehicleRespawnEnabled)) then {
										...
										0 = [6,"Vehicle Respawn online",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
										if (alive ((_x getVariable 'QS_transporter') # 1)) then {
											0 = (missionNamespace getVariable 'QS_leaderboards_session_queue') pushBack ['TRANSPORT',((_x getVariable 'QS_transporter') # 2),((_x getVariable 'QS_transporter') # 0),4];
										};
									};
				...
				*/
							...
							if ((getMass _x) > 5000) then {
								if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
									if (!(_module_fob_logistics_repairServices)) then {
										...
										0 = [6,"Vehicle Repair Service online",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
										if (!isNil {_x getVariable 'QS_transporter'}) then {
				...
				if (!(_module_fob_logistics_ammoServices)) then {
					{
						if ((['ammo',(typeOf _x),_false] call _fn_inString) || {(_x isKindOf 'Reammobox_F') && !(['repair',(typeOf _x),_false] call _fn_inString)} || {(_x isKindOf 'B_supplyCrate_F')}) then {
							if ((getMass _x) > 2000) then {
								if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
									if (isNil {_x getVariable 'QS_vehicle_isSuppliedFOB'}) then {
										if (!(_module_fob_logistics_ammoServices)) then {
											...
											0 = [6,"Vehicle Rearm Service & Arsenal online",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
											if (!isNil {_x getVariable 'QS_transporter'}) then {
												if (alive ((_x getVariable 'QS_transporter') # 1)) then {
													0 = (missionNamespace getVariable 'QS_leaderboards_session_queue') pushBack ['TRANSPORT',((_x getVariable 'QS_transporter') # 2),((_x getVariable 'QS_transporter') # 0),4];
												};
								...
								if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
									if (isNil {_x getVariable 'QS_vehicle_isSuppliedFOB'} && {isObjectHidden (missionNamespace getVariable ['QS_module_fob_supplycrate', objNull])}) then {
										(missionNamespace getVariable 'QS_module_fob_supplycrate') hideObjectGlobal FALSE;
										/*
										...
										*/
										//_x setVariable ['QS_vehicle_isSuppliedFOB',_true,_true];
										0 = [6,"FOB: Arsenal online",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
										if (!isNil {_x getVariable 'QS_transporter'}) then {
											if (alive ((_x getVariable 'QS_transporter') # 1)) then {
												0 = (missionNamespace getVariable 'QS_leaderboards_session_queue') pushBack ['TRANSPORT',((_x getVariable 'QS_transporter') # 2),((_x getVariable 'QS_transporter') # 0),4];
											};
						...
						if (['fuel',(typeOf _x),_false] call _fn_inString) exitWith {
							if ((isNull (attachedTo _x)) && (isNull (ropeAttachedTo _x))) then {
								if (!(_module_fob_logistics_fuelServices)) then {
									...
									//(missionNamespace getVariable 'QS_module_fob_repairDepot') hideObjectGlobal FALSE;
									//0 = ['FOB_UPDATE',['','Vehicle Service online']] remoteExec ['QS_fnc_showNotification',-2,_false];
									0 = [6,"Vehicle Refuel Service online",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
									if (!isNil {_x getVariable 'QS_transporter'}) then {
										if (alive ((_x getVariable 'QS_transporter') # 1)) then {
											0 = (missionNamespace getVariable 'QS_leaderboards_session_queue') pushBack ['TRANSPORT',((_x getVariable 'QS_transporter') # 2),((_x getVariable 'QS_transporter') # 0),4];
										};
	...
	if (_timeNow > _vRespawn_checkDelay) then {
		...
		};
		{
			if(isTouchingGround _x) then {
				_x setVariable ["McD_vehicleIsTouchingGround", nil, TRUE];
			} else {
				_x setVariable ["McD_vehicleIsTouchingGround", FALSE, TRUE];
			};
		} foreach (vehicles select {
			local _x &&
			{!isNil {_x getVariable "McD_vehicleIsTouchingGround"}}
		});
		_vRespawn_checkDelay = time + _vRespawn_delay;
	};
			...
			if (!(allUnitsUav isEqualTo [])) then {
				{
					...
					if (local _uav) then {
						if (!(_uav getVariable ['QS_uav_protected',_false])) then {
							if !(isNull (isVehicleCargo _uav)) then {
								objNull setVehicleCargo _uav;
							};
							if !(isNull (ropeAttachedTo _uav)) then {
								// don't detach rope, just try to delete
							};
						...
						if (_QS_module_emergentTask_instruction isEqualTo 'ADD') then {
							...
							if ((_QS_module_emergentTask_logic_created # 0) call (_QS_module_emergentTask_logic_created # 1)) then {
								...
								if (_QS_module_emergentTask_countType < _QS_module_emergentTask_maxType) then {
									...
									if (_QS_module_emergentTask_type isEqualTo "MEDEVAC") then {
										// Add Timer to player initiated medevac missions
										[_QS_module_emergentTask_id,TRUE,serverTime + 590] call (missionNamespace getVariable 'QS_fnc_taskSetTimer');
									};
									//comment 'Add to eval';
