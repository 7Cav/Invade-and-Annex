...
if (!isNull _vehicle) then {
	...
	if (_type in ['o_uav_06_f','o_uav_06_medical_f','o_uav_01_f']) then {
		...
		// McDodelijk Edit Start
		// Make sure there is at least one valid location to allow AIHandleGroup to not crash in line 524 due to missing location
		if (_radialPatrolPositions isEqualTo []) then {
			_centerPos set [2,75];
			_radialPatrolPositions pushBack _centerPos;
		};
		// McDodelijk Edit End
		if (!(_radialPatrolPositions isEqualTo [])) then {
