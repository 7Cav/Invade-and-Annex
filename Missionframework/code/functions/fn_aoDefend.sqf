...
if (((random 1) > 0.4) && ((missionNamespace getVariable 'QS_forceDefend') isEqualTo 0)) exitWith {missionNamespace setVariable ['QS_defendActive',FALSE,TRUE];};
...
_defendMessages = [
	...
	'Dig in at the HQ boys, they are almost here!',
	'Its a trap! CSAT is all around us!',
	'CSAT is coming for all of us! Hide!'
];
...
_durationAlmostOver = _duration - 120 - (random 120);
...
_exitSuccess = FALSE;
_exitFail = FALSE;
_checkHeldInitialDelay = time + 45;
...
_QS_infantry = TRUE;
_infantryInitialSpawnDelay = time + 45;
...
_jetType = selectRandomWeighted [
	...
	//'o_plane_fighter_02_f',0.05
];
_jetArray = [];
_jetInitialDelay = time + (45 + (random 60));
_jet = objNull;
_helicopters = TRUE;
_helicoptersToSpawn = 1;
if (_allPlayersCount > 0) then {_helicoptersToSpawn = 0;};
if (_allPlayersCount > 10) then {_helicoptersToSpawn = 1;};
if (_allPlayersCount > 20) then {_helicoptersToSpawn = selectRandom [1,1,2];};
...
if (_allPlayersCount > 20) then {
	if (worldName in ['Tanoa','Enoch']) then {
		_helicopterTypes = [
			'o_heli_light_02_dynamicloadout_f','i_e_heli_light_03_dynamicloadout_f','i_heli_light_03_dynamicloadout_f','O_Heli_Attack_02_dynamicLoadout_F','O_Heli_Attack_02_dynamicLoadout_black_F','O_T_VTOL_02_vehicle_dynamicLoadout_F','O_T_VTOL_02_infantry_dynamicLoadout_F'
		];
	} else {
		_helicopterTypes = [
			'o_heli_light_02_dynamicloadout_f','i_heli_light_03_dynamicloadout_f',
			'o_heli_light_02_dynamicloadout_f','i_heli_light_03_dynamicloadout_f',
			'O_Heli_Attack_02_dynamicLoadout_black_F','O_Heli_Attack_02_dynamicLoadout_F','O_T_VTOL_02_vehicle_dynamicLoadout_F','O_T_VTOL_02_infantry_dynamicLoadout_F'
		];
	};
} else {
	_helicopterTypes = [
		'o_heli_light_02_dynamicloadout_f',(['i_heli_light_03_dynamicloadout_f','i_e_heli_light_03_dynamicloadout_f'] select (worldName in ['Tanoa','Enoch'])),'i_heli_light_03_dynamicloadout_f','O_Heli_Attack_02_dynamicLoadout_black_F','O_T_VTOL_02_vehicle_dynamicLoadout_F','O_T_VTOL_02_infantry_dynamicLoadout_F'
	];
};
_helicopterType = '';
_helicopterArray = [];
...
_paratrooperType = '';
_paratrooperArray = [];
_paratrooperInitialDelay = time + 240 + (random 100);
_paratrooper2InitialDelay = _paratrooperInitialDelay + 180 + (random 100);
...
_vParaArray = [];
_vParaToSpawn = 0;
if (_allPlayersCount > 0) then {_vParaToSpawn = 2;};
if (_allPlayersCount > 10) then {_vParaToSpawn = 2;};
if (_allPlayersCount > 20) then {_vParaToSpawn = 3;};
if (_allPlayersCount > 30) then {_vParaToSpawn = 3;};
if (_allPlayersCount > 40) then {if ((random 1) > 0.666) then {_vParaToSpawn = 3;} else {_vParaToSpawn = 4;};};
if (_allPlayersCount > 50) then {_vParaToSpawn = 4;};
_QS_flyBy = FALSE;
if ((random 1) > 0.333) then {
	_QS_flyBy = TRUE;
};
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_exitSuccess) exitWith {
		...
		[1,"HQ defended",0,70] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if (_exitFail) exitWith {
		...
		[2,"Defence failed",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
