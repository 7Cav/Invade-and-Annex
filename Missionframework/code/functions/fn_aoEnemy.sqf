...
_infUrbanTypes = [
	...
	'I_E_ShotG',0.333,
	'OIA_InfTeam_AA',0.333
];
if (worldName in ['Tanoa','Lingor3']) then {
	_staticTypes = ['O_HMG_01_high_F'];
	_airTypes = ['i_heli_light_03_dynamicloadout_f','O_Heli_Light_02_dynamicLoadout_F','O_Heli_Attack_02_dynamicLoadout_black_F','O_T_VTOL_02_vehicle_dynamicLoadout_F','O_T_VTOL_02_infantry_dynamicLoadout_F'];
	...
	_airTypes = ['i_heli_light_03_dynamicloadout_f','O_Heli_Light_02_dynamicLoadout_F','O_Heli_Attack_02_dynamicLoadout_black_F','O_Heli_Attack_02_dynamicLoadout_F','O_T_VTOL_02_vehicle_dynamicLoadout_F','O_T_VTOL_02_infantry_dynamicLoadout_F'];
...
if (_playerCount > 0) then {
	...
	if ((random 1) > 2) then {
		for '_x' from 1 to _aaCount step 1 do {
			...
			{
			  _x addCuratorEditableObjects [[_aa], TRUE];
			} count allCurators;
...
_commander addEventHandler [
	'Killed',
	{
		...
		if!(_killed getVariable ["QS_isSurrendered", FALSE]) then {
			[1,"CSAT commander killed",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
