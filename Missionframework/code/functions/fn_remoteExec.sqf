...
if (_case < 10) exitWith {
	...
	if (_case isEqualTo 8) then {
		...
		if (local _t && _t == player) then {
			disableUserInput TRUE;
		};
		_t setAnimSpeedCoef 1.5;
		_t switchMove _anim;
		uisleep 4;
		_t setAnimSpeedCoef 1;
		if (local _t && _t == player) then {
			disableUserInput FALSE;
		};
		if ((lifestate _t) isEqualTo "INCAPACITATED" && {isNull objectParent _t}) then {
			private _e1 = eyePos player;
			_t switchMove "acts_InjuredLyingRifle02";
			if (local _t) then {
				uisleep 0.01;
				_t setDir _dir;
				_t setPosASL (getPosASL _t vectorAdd (_e1 vectorDiff (eyePos player)));
			};
		};
	};
	/*/ release drag/*/
	...
	if (_case isEqualTo 21) then {
		...
		if (local _unit) then {
			...
			if (isPlayer _unit) then {
				...
				if(_unit isEqualTo (missionNamespace getVariable ['QS_csatCommander', objNull])) then {
					missionNamespace setVariable ['QS_csatCommander', _agent, TRUE];
				};
...
if (_case < 60) exitWith {
	...
	if (_case isEqualTo 58) then {
		if (isDedicated) then {
			...
			[1,"Enemy commander captured",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
...
if (_case < 80) exitWith {
	...
	if (_case isEqualTo 73) then {
				...
				private _centroid = missionNamespace getVariable ['QS_aoPos',[0,0,0]];
				{
					...
					if ((side _unit) in [EAST,RESISTANCE]) then {
						if ((_unit distance2D _centroid) < 1500) then {
							_posArray pushBack (position _unit);
						};
					};
				} forEach allUnits;
				[_posArray] remoteExec ['McD_fnc_markEnemyPosTemp', -2, FALSE];
				[1,"Datalink secured",0,10] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		...
		if (_type isEqualTo 3) then {
			...
			[1,"Supply Depot secured",0,10] remoteExec ['NL_fnc_objPopup', -2, FALSE];
			[10] call NL_fnc_addScore;
		};
	};
	/*/===== CAS spawner/*/
