...
if (_type isEqualTo 1) exitWith {
	...
	if (_subType isEqualTo 'INTEL') then {
		...
		if (_positionFound) then {
			...
			_description = 'Locate and secure the enemy datalink.<br/><br/>The enemy datalink allows the enemy to more easily communicate and share information on our troop movements, strength and force disposition. With this data they can adapt and better counter our attacks.<br/><br/> This datalink also allows them to call in UAV recon support. Securing the datalink will deny the enemy these benefits. <br/><br/> If we capture it we will be able to gain information about enemy movement for a short while. It is not advised to capture it before taking out the GPS Jammer.';
...
if (_type isEqualTo 2) exitWith {
	...
	_return = TRUE;
	if (_subType isEqualTo 'INTEL') then {
		_return = !(missionNamespace getVariable ['QS_virtualSectors_sub_1_active', TRUE]);
	};
	if (_subType isEqualTo 'VEHICLE') then {
		_return = TRUE;
	};
	if (_subType isEqualTo 'GEAR') then {
		_return = !(missionNamespace getVariable ['QS_virtualSectors_sub_3_active', TRUE]);
	};
	if (_subType isEqualTo 'SUPPORT') then {
		_return = TRUE;
	};
