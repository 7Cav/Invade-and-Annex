...
private [
	...
	'_QS_decayRate','_QS_decayDelay','_QS_fpsCheckDelay','_QS_fpsDelay','_pilotCheck','_QS_action_serviceVehicle','_QS_action_serviceVehicle_vehicle','_QS_action_serviceVehicle_text',
...
private _roleSelectionSystem = missionNamespace getVariable ['QS_RSS_enabled',TRUE];
...
_fn_manDragDistanceCheck = {
	params [["_target", objNull, [objNull]]];
	if(isNull _target) exitWith {FALSE};
	private _rel = _target worldToModel (ASLToAGL eyePos player);
	private _min = 1.45;
	private _closeEnough = FALSE;
	{
		if((_rel distance (_target selectionPosition _x)) < _min) exitWith { _closeEnough = TRUE; };
	} foreach [
		"neck",
...
_QS_action_serviceVehicle_vehicle = objNull;
...
/*/== Refuel with toolkit/*/
private _McD_interaction_vToolkitRefuel = FALSE;
private _McD_action_vToolkitRefuel = nil;
private _McD_vToolkitRefuel = (missionNamespace getVariable "McD_fnc_vToolkitRefuel");
/*/== Block tactical ping/*/
private _McD_tacticalPingBlock_mode = (missionNamespace getVariable ["McD_missionConfig_tacticalPing_blockingMode", 0]);
private _McD_tacticalPingBlock_array = ['', {}, nil, -100, TRUE, FALSE, 'TacticalPing', 'TRUE'];
private _McD_tacticalPingBlock_active = _false;
private _McD_tacticalPingBlock_action = -1;
/*/== Revive Anim/*/
...
/*/===== Ear collector/*/
_QS_action_ears = nil;
_QS_action_ears_text = 'Collect Dog tag';
...
_QS_action_teeth_text = 'Collect Beret';
...
_QS_module_safezone_speedlimit_enabled = FALSE;
...
if (isNil {player getVariable 'QS_ClientBTexture2'}) then {
	player setVariable ['QS_ClientBTexture2','',FALSE];
};
if (isNil {player getVariable 'QS_ClientBTexture2_Backpacks2'}) then {
...
0 spawn {
	// Initialize dynamic groups with registering the own group and then leaving it to prevent certain problems(getting pulled in open squads, regaining sl from another squad)
	["InitializePlayer", [player, true]] call BIS_fnc_dynamicGroups;
	sleep 1;
	["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call BIS_fnc_dynamicGroups;
};
...
x' from 0 to 1 step 0 do {
	...
	if (alive _QS_player) then {
		if (_lifeState in ['HEALTHY','INJURED']) then {
			...
			if (
				...
				if (
					(_cursorDistance < 3) &&
					{_cursorTarget isKindOf 'Man'}
				) then {
					if (
						(alive _cursorTarget) &&
						{(lifeState _cursorTarget) isEqualTo 'INCAPACITATED'} &&
						{isNull (objectParent _cursorTarget)} &&
						{_cursorTarget call _fn_manDragDistanceCheck}
					) then {
			...
			/*/===== Vehicle Service/*/
			if (
				(!_noObjectParent || unitIsUav _QS_cO) && { ( (_QS_cO isKindOf 'LandVehicle') || (_QS_cO isKindOf 'Air') ) } &&
				{(!(missionNamespace getVariable ['QS_repairing_vehicle',_false]))} &&
				{(((vectorMagnitude (velocity _QS_cO)) * 3.6) <= 1)} &&
				{(local _QS_cO) || {(isNull (driver _QS_cO) && assignedVehicleRole _QS_player isEqualTo ["Turret", [0]])} }
			) then {
				_isNearRepairDepot = ([_QS_cO] call _fn_isNearRepairDepot);
				_nearSite = _false;
				{
					if ((_QS_cO distance2D (markerPos _x)) < 12) exitWith {
						_nearSite = _true;
					};
				...
				if (
					(_nearSite) ||
					{(_isNearRepairDepot)} ||
					{(['all', _QS_cO] call QS_fnc_checkServiceVehicles)} ||
					{((!(_QS_carrierEnabled isEqualTo 0)) && (['INPOLYGON',_QS_cO] call _fn_carrier))} ||
					{((!(_QS_destroyerEnabled isEqualTo 0)) && (['INPOLYGON',_QS_cO] call _fn_destroyer))}
				) then {
					if (!(_QS_interaction_serviceVehicle)) then {
						...
						_QS_action_serviceVehicle = _QS_cO addAction _QS_action_serviceVehicle_array;
						_QS_action_serviceVehicle_vehicle = _QS_cO;
						_QS_cO setUserActionText [_QS_action_serviceVehicle,((_QS_cO actionParams _QS_action_serviceVehicle) select 0),(format ["<t size='3'>%1</t>",((_QS_cO actionParams _QS_action_serviceVehicle) select 0)])];
					};
				} else {
					if (_QS_interaction_serviceVehicle) then {
						_QS_interaction_serviceVehicle = _false;
						_QS_action_serviceVehicle_vehicle removeAction _QS_action_serviceVehicle;
					};
				};
			} else {
				if (_QS_interaction_serviceVehicle) then {
			...
			/*/===== Refuel vehicle as engineer with toolkit/*/

			if ([0] call _McD_vToolkitRefuel) then {
				if (!(_McD_interaction_vToolkitRefuel)) then {
					_McD_interaction_vToolkitRefuel = _true;
					_McD_action_vToolkitRefuel = [1] call _McD_vToolkitRefuel;
					player setUserActionText [_McD_action_vToolkitRefuel,((player actionParams _McD_action_vToolkitRefuel) select 0),(format ["<t size='3'>%1</t>",((player actionParams _McD_action_vToolkitRefuel) select 0)])];
				};
			} else {
				if (_McD_interaction_vToolkitRefuel) then {
					_McD_interaction_vToolkitRefuel = _false;
					player removeAction _McD_action_vToolkitRefuel;
				};
			};

			/*/===== Block tactical ping/*/
			// As with shooting in base, this is not sufficant to stop pings
			// Key eventhandler is adding another layer of 'security'
			if ( _McD_tacticalPingBlock_mode != 0) then {
				if (
					_McD_tacticalPingBlock_mode == -1 ||
					{_McD_tacticalPingBlock_mode == 1 && {player != (leader group player) }} ||
					{((count (units (group player))) < 2)}
				) then {
					if !(_McD_tacticalPingBlock_active) then {
						_McD_tacticalPingBlock_action = player addAction _McD_tacticalPingBlock_array;
						_McD_tacticalPingBlock_active = _true;
					};
				} else {
					if (_McD_tacticalPingBlock_active) then {
						player removeAction _McD_tacticalPingBlock_action;
						_McD_tacticalPingBlock_active = _false;
					};
				};
			};
			...
			if (
				...
				{(_cursorObject getVariable ['QS_arsenal_object',_false])} &&
			...
			if (
				...
				{(_cursorObject getVariable ['QS_arsenal_object',_false])} &&
				{(((vectorMagnitude (velocity _QS_player)) * 3.6) < 1)} &&
				{! (cursorObject isEqualTo (missionNamespace getVariable ['QS_module_fob_supplycrate', objNull]))}
			) then {
			...
			*/
			...
			if (!(((attachedObjects _QS_player) findIf {((!isNull _x) && ((_x isKindOf 'Man') || {([0,_x,_objNull] call _fn_getCustomCargoParams)} || {(_x isKindOf 'StaticWeapon')} || {(_x getVariable ['QS_RD_draggable',_false])}))}) isEqualTo -1)) then {
				{
					if ((_x isKindOf 'Man') || {([0,_x,_objNull] call _fn_getCustomCargoParams)} || {(_x isKindOf 'StaticWeapon')} || {(_x getVariable ['QS_RD_draggable',_false])}) then {
	...
	if (!isNil {_QS_player getVariable 'QS_RD_interacting'}) then {
		if (_QS_player getVariable 'QS_RD_interacting') then {
			if (!isNil {_QS_player getVariable 'QS_RD_carrying'}) then {
				if (_QS_player getVariable 'QS_RD_carrying') then {
					private _attachedObjects = (attachedObjects _QS_player) select {!(_x isKindOf 'Sign_Sphere10cm_F')};
				...
				if (!isNil {_QS_crewIndicatorUI}) then {
					...
					if (!isNull _objectParent) then {
						...
						if (!(_fullCrew isEqualTo [])) then {
							if (({(alive (_x # 0))} count _fullCrew) > 0) then {
								_crewManifest = '';
								/*
								...
								*/
								// =========== McDodelijks replacement start
								_fnc_parseUnit = {
									params ['_unit','_role','','_turretPath','_personTurret'];
									private _unitName = name _unit;
									private _roleImg = switch (toUpper _role) do {
										case 'DRIVER': {_QS_crewIndicator_imgDriver};
										case 'COMMANDER': {_QS_crewIndicator_imgCommander};
										case 'GUNNER': {_QS_crewIndicator_imgGunner};
										case 'TURRET': {[_QS_crewIndicator_imgGunner, _QS_crewIndicator_imgCargo] select _personTurret};
										case 'CARGO': {_QS_crewIndicator_imgCargo};
										default {_QS_crewIndicator_imgCargo};
									};
									if (!((toUpper _role) isEqualTo 'DRIVER')) then {
										if (!(_turretPath isEqualTo [])) then {
											_text = format ["<img image=%1/><t size='0.666'>%2</t> <t size='0.666'>%3</t><br/>",_roleImg,(_turretPath select 0),_unitName];
										} else {
											_text = format ["<img image=%1/> <t size='0.666'>%2</t> <br/>",_roleImg,_unitName];
										};
									} else {
										_text = format ["<img image=%1/> <t size='0.85'>%2</t><br/>",_roleImg,_unitName];
									};
									_text;
								};
								// Start with the pilot
								private _d = (_fullCrew select {(toUpper (_x # 1)) == 'DRIVER'}) # 0;
								if !(isNil "_d") then {
									_crewManifest = _d call _fnc_parseUnit;
									_fullCrew = _fullCrew - [_d];
								};
								// Sort rest by squad
								while {count _fullCrew > 0} do {
									private _g = groupId group (_fullCrew # 0 # 0);
									_crewManifest = _crewManifest + format ["<t size='0.85'>%1</t><br/>",_g];
									private _gm = _fullCrew select {(groupId group (_x # 0)) == _g};
									{_crewManifest = _crewManifest + (_x call _fnc_parseUnit);} foreach _gm;
									_fullCrew = _fullCrew - _gm;
								};

								// =========== McDodelijks replacement end
	...
	if (_QS_module_gearManager) then {
		if (missionNamespace getVariable ['QS_client_triggerGearCheck',_false]) then {
			...
			[_QS_player] call _fn_gearRestrictions;
		};
	};
	...
	if (_QS_module_safezone) then {
		if (_timeNow > _QS_module_safezone_checkDelay) then {
			_allPlayers = allPlayers;
			if (
				((_QS_player distance2D _QS_module_safezone_pos) < _QS_module_safezone_radius) &&
				!(
					(!isNil {_QS_v2 getVariable 'NL_vehicle_baseAA'}) ||
					((!isNil {(_QS_v2 getVariable 'NL_vehicle_baseCAS')}) && {!isTouchingGround _QS_v2}) ||
					{(_QS_v2 isEqualTo (missionNamespace getVariable 'QS_arty'))}
				)
			) then {
				...
				if (!(_QS_safezone_action in (actionIDs player))) then {
					...
					_QS_player setUserActionText [_QS_safezone_action,((player actionParams _QS_safezone_action) # 0),'',''];
				};
				...
				if (_QS_module_safezone_isInSafezone) then {
					...
					if (_QS_module_safezone_playerProtection isEqualTo 1) then {
						...
						50 cutText ['Exiting safezone','PLAIN DOWN',0.25];
						_QS_safezone_action = -1;
						if (!isDamageAllowed _QS_player) then {
	...
	if (_QS_v2TypeL in ['b_apc_tracked_01_crv_f','b_t_apc_tracked_01_crv_f']) then {
		if (_QS_player isEqualTo (driver _QS_v2)) then {
			if ((_QS_v2 animationSourcePhase 'MovePlow') isEqualTo 1) then {
				...
				{
					if (!isNull (_x # 3)) then {
						if ((_x # 3) isKindOf 'AllVehicles' || (_x # 3) isKindOf 'Slingload_base_f' || (_x # 3) isKindOf 'StaticWeapon') then {
							if (!alive (_x # 3)) then {
				...
				if (!(_listOfFrontStuff isEqualTo [])) then {
					...
					_QS_toDelete = [];
					{
						...
						if (_x in allDead || _x isKindOf 'Slingload_base_f') then {
	...
	if (_QS_module_radioChannelManager) then {
		if (_timeNow > _QS_module_radioChannelManager_checkDelay) then {
			_QS_module_radioChannelManager_notPilot = ((!(_QS_player getUnitTrait 'QS_trait_pilot')) && (!(_QS_player getUnitTrait 'uavhacker')) && (!(_QS_player getUnitTrait 'QS_trait_HQ')) && (!(_QS_player getUnitTrait 'QS_trait_fighterPilot')) && (!(_QS_player getUnitTrait 'QS_trait_CAS')) && (!(_QS_player getUnitTrait 'QS_trait_JTAC')));
			// leader are allowed on command channel
			if( ["PlayerIsLeader", [_QS_player]] call (missionNamespace getVariable 'BIS_fnc_dynamicGroups') || (McD_adminForcedRadioChannels # 0)) then {

				if !(player getVariable ["McD_commandChannel", FALSE]) then {
					player setVariable ["McD_commandChannel", TRUE, [2,clientOwner]];
				};
				if !(channelEnabled (_QS_module_radioChannelManager_commandChannel + 5) isEqualTo [true, true]) then {
					(_QS_module_radioChannelManager_commandChannel + 5) enableChannel [true, true];
					[WEST,'BLU'] sideChat "Command channel is now fully available to you";
				};
			} else {
				// ungrouped is leader of his own squad - let him hear and write but not talk
				if (_QS_player == (leader group _QS_player)) then {

					if !(player getVariable ["McD_commandChannel", FALSE]) then {
						player setVariable ["McD_commandChannel", TRUE, [2,clientOwner]];
					};
					if !(channelEnabled (_QS_module_radioChannelManager_commandChannel + 5) isEqualTo [true, false]) then {
						(_QS_module_radioChannelManager_commandChannel + 5) enableChannel [true, false];
						[WEST,'BLU'] sideChat "You have restricted access to the Command channel";
					};
				} else {
					// part of group - make sure to remove channel
					if !(channelEnabled (_QS_module_radioChannelManager_commandChannel + 5) isEqualTo [false, false]) then {
						(_QS_module_radioChannelManager_commandChannel + 5) enableChannel [false, false];
						[WEST,'BLU'] sideChat "You can't communicate over Command channel anymore";
					};
					if (player getVariable ["McD_commandChannel", FALSE]) then {
						player setVariable ["McD_commandChannel", FALSE, [2,clientOwner]];
					};
					if (currentChannel == (_QS_module_radioChannelManager_commandChannel + 5)) then {
						setCurrentChannel 5;
					};
				};
			};
			...
			if (_QS_module_radioChannelManager_notPilot) then {
				if ([_atcMarkerPos,_tocMarkerPos] call _QS_module_radioChannelManager_checkState || (McD_adminForcedRadioChannels # 1)) then {
		...
		if (!(_QS_module_49Opened)) then {
			...
			if (!isNull _d49) then {
				...
				*/
				...
				(_d49 displayCtrl 103) ctrlSetText 'Suicide';
				(_d49 displayCtrl 103) ctrlSetTooltip 'Die on the spot';
				(_d49 displayCtrl 103) buttonSetAction "[] spawn { waitUntil {isNull (findDisplay 49)}; _result = ['Do you really need to kill yourself?','','Suicide','Cancel',(findDisplay 46)] call (missionNamespace getVariable 'BIS_fnc_guiMessage'); if (_result) then {if ( ((lifeState player) in ['HEALTHY','INJURED']) && (scriptDone (missionNamespace getVariable 'QS_script_incapacitated')) ) then {player allowDamage FALSE; missionNamespace setVariable ['QS_script_incapacitated',([player , '', '', '', '', '', player] spawn (missionNamespace getVariable 'QS_fnc_incapacitated')),FALSE];};};};";
				(_d49 displayCtrl 103) ctrlEnable _true;
				...
				(_d49 displayCtrl 104) ctrlSetTooltip 'Go to lobby';

				(_d49 displayCtrl 1010) ctrlEnable FALSE;
				(_d49 displayCtrl 1010) ctrlSetText 'Respawn';
				(_d49 displayCtrl 1010) ctrlSetTooltip 'Respawn back to base';
				(_d49 displayCtrl 1010) buttonSetAction 'player setVariable ["QS_revive_respawnType","BASE", FALSE]; player setVariable ["McD_RespawnWatch_alive", TRUE, FALSE]; player setVariable ["McD_RespawnWatch_RespawnPosition", position player, FALSE];';
	...
	if (_QS_module_soundControllers) then {
		if (_QS_uiTime > _QS_module_soundControllers_checkDelay) then {
			_QS_player setVariable ['QS_client_soundControllers',[(if(_QS_v2 isKindOf "Man") then {[]} else {(getAllSoundControllers _QS_v2)}),(getAllEnvSoundControllers _QS_posWorldPlayer)],_false];
