	...
	if (_aoPos inPolygon [
		...
		if (!(_buildingList isEqualTo [])) then {
			...
			if (!isNull _building) then {
				...
				[0,"Arrest HVT",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
				[
					'QS_IA_TASK_AO_3',
					TRUE,
					[
						'Intel teams have located a High Value Target in the area. Move in and arrest him so we can finish the AO!',
						'Arrest HVT',
						'Arrest HVT'
					],
					[_agent,TRUE],
...
if (_state isEqualTo 2) then {
	...
	if (missionNamespace getVariable ['QS_aoSmallTask_Arrested',FALSE]) then {
		...
		[1,"HVT arrested",0,20] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		...
		if (!alive _agent) then {
			...
			[2,"HVT killed",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
			['QS_IA_TASK_AO_3'] call (missionNamespace getVariable 'BIS_fnc_deleteTask');
			_return = [
				_case,
				0,
