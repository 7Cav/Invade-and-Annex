...
if (_worldName isEqualTo 'Altis') then {
	...
	_QS_loiterPos = (getmarkerpos "NL_spawnUAV");
};
	...
	if (_casEnabled) then {
		...
		if ((count _QS_spawnedUAVs) < _QS_maxUAVs) then {
			if (missionNamespace getVariable ['QS_uavCanSpawn',FALSE]) then {
				...
				_QS_uav = (selectRandom _QS_uavTypesAllowed) createVehicle _QS_loiterPos;
				_QS_uav allowDamage FALSE;
				/* _QS_uav = createVehicle [(selectRandom _QS_uavTypesAllowed),_QS_loiterPos,[],0,'NONE']; */
				_QS_uav setDir 223;
				_QS_uav engineOn FALSE;
				...
				_text = format ['A(n) %1 has spawned at base',(getText (configFile >> 'CfgVehicles' >> (typeOf _QS_uav) >> 'displayName')),(mapGridPosition _QS_uav),worldName];
			...
			if (missionNamespace getVariable ['QS_drone_heli_enabled',FALSE]) then {
				if ((isNull _QS_heliDrone) || {(!alive _QS_heliDrone)}) then {
					...
					if (_carrierEnabled) then {
						...
						_QS_heliDrone setVariable ['NL_vehicle_baseUAV',TRUE,TRUE];
