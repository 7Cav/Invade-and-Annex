...
// Jam Setting for MAgatine jam by McDo
if !(MD_exclusiveHighCapMags_jammed) then {
    if (_magazine in MD_exclusiveHighCapMags_greyMags) then {
        if !(getText(configFile >> "CfgWeapons" >> _weapon >> "baseWeapon") in MD_exclusiveHighCapMags_whiteRifles) then {
            if((random 1) < MD_exclusiveHighCapMags_jamChance) then {
                MD_exclusiveHighCapMags_jammed = true;
                0 spawn {
                    _f = diag_frameno;
                    _w = currentWeapon player;
                    _a = player ammo _w;
                    player setAmmo [_w, 0];
                    waitUntil {_f < diag_frameno};
                    player setAmmo [_w, _a];
                    cutText ["<t color='#ff0000' size='2'>Weapon jammed! Reload!</t><br/>The high capacity magazines do not work that well with this gun...", "PLAIN DOWN", -1, false, true];
                    uisleep 5;
                    cutText ["", "PLAIN DOWN"];
                };
            };
