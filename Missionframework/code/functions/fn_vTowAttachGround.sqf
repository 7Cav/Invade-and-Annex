...
params ['_vehicle','_towedVehicle','_isUAV', '_oldLock'];
//_ropeLength = ((((boundingBoxReal _vehicle) select 1) select 1) / 1.5) + 1 + (((boundingBoxReal _towedVehicle) select 1) select 1);
_ropeLength = (((boundingBoxReal _vehicle) select 1) select 1) + 0.4 + (((boundingBoxReal _towedVehicle) select 1) select 1);
...
private _defaultAttachPoint = [[0,-_ropeLength, abs (boundingBoxReal _towedVehicle # 0 # 2) - abs (boundingBoxReal _vehicle # 0 # 2) + 0.4 ],0];
...
waitUntil {
	...
	(
		...
		((!(unitIsUAV _towedVehicle)) && {!(_towedVehicle isKindOf 'StaticWeapon')} && {(({alive _x} count (crew _towedVehicle)) > 0)}) ||
		( (unitIsUAV _towedVehicle) && {({if(_x isEqualType objNull) then {FALSE} else {if(_x isEqualTo "") then {FALSE} else {TRUE};};} count (UAVControl _towedVehicle)) > 0} )
	)
};
...
if (isNil {_towedVehicle getVariable 'QS_loadCargoIn'}) then {
	...
	if (!(_posIsClear)) then {
		systemChat str [(_towedVehicle call (missionNamespace getVariable 'QS_fnc_isBoundingBoxIntersected')), _line01, _line02, _line03];
	...
	*/
	...
	// New try by McD with working collision prevention. Not working that great because findEmptyPosition is not working as good.
	// Also quicksilver tried something similar, it was always overwritten. See commented block near end of file.
	/*if (_posIsClear) then {
		isNil {
			_towedVehicle setVehiclePosition [_towedVehicle, [], 0, "CAN_COLLIDE"];
			detach _towedVehicle;
			_towedVehicle setVehiclePosition [_towedVehicle, [], 0, "CAN_COLLIDE"];
		};
	};
	...
	if!(_posIsClear) then {
		systemChat str [(_towedVehicle call (missionNamespace getVariable 'QS_fnc_isBoundingBoxIntersected')), (_line01 isEqualTo []), (_line02 isEqualTo []), (_line03 isEqualTo [])];
		private _center = getPosWorld _towedVehicle;
		private _position = [];
		for '_x' from 2 to 30 step 2 do {
			_position = _center findEmptyPosition [0,_x,(typeOf _towedVehicle)];
			if !(_position isEqualTo []) then {
				if ((lineIntersectsSurfaces [(AGLToASL (_vehicle modelToWorld [0,0,0])),(AGLToASL _position),_vehicle,_towedVehicle,TRUE,1,'GEOM']) isEqualTo []) exitWith {};
			};
		};
		isNil {
			if( (_position isEqualTo []) || {!(_towedVehicle setVehiclePosition [_position, [], 0, "none"])} ) then {
				if( (_position isEqualTo []) || {!(_towedVehicle setVehiclePosition [_position, [], 30, "none"])} ) then {
					_towedVehicle setVehiclePosition [_towedVehicle, [], 0, "CAN_COLLIDE"];
				};
			};
			detach _towedVehicle;
	...
	// Just release the thing where it is, but try putting it on ground
	isNil {
		_towedVehicle setVehiclePosition [_towedVehicle, [], 0, "CAN_COLLIDE"];
		detach _towedVehicle;
		_towedVehicle setVehiclePosition [_towedVehicle, [], 0, "CAN_COLLIDE"];
...
if (local _towedVehicle) then {
	_towedVehicle lock _oldLock;
} else {
	['lock',_towedVehicle, _oldLock] remoteExec ['QS_fnc_remoteExecCmd',_towedVehicle,FALSE];
};
if (isNull (attachedTo _towedVehicle)) then {
