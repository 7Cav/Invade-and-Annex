			...
			if (unitIsUAV _killer) then {
				if (!local _killer) then {
					if (([(getPosATL _killer),15,[WEST,CIVILIAN],allPlayers,0] call (missionNamespace getVariable 'QS_fnc_serverDetector')) isEqualTo []) then {
						diag_log format ["Note: eventEntityKilled wants to delete a %1 at %2. This is prevented for Alx and his Praetorian guns. Any issues? Get in touch with McDodelijk", typeOf _killer, (getPosATL _killer)];
