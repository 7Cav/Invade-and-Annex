...
_QS_ST_enableGroupIcons = TRUE; /*player getUnitTrait 'QS_trait_HQ';*/							// BOOL. TRUE to enable Map+GPS+HUD GROUP Icons. Default TRUE.
McD_fnc_enableGroupIconsCheck = compileFinal "player getUnitTrait 'QS_trait_HQ' || {['PlayerIsLeader', [player]] call (missionNamespace getVariable 'BIS_fnc_dynamicGroups')}";	// FUNCTION returning BOOL
...
if (_QS_ST_X # 2) then {
	...
	_grpscript = [_QS_ST_X] spawn {
		...
		for '_x' from 0 to 1 step 0 do {
			if (0 call McD_fnc_enableGroupIconsCheck) then {
				if (_dynamicDiplomacy) then {
