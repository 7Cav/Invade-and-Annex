		...
		if (alive _weapon) then {
			if ((_weapon isKindOf 'StaticWeapon') || {(_weapon isKindOf 'StaticMortar')}) then {
				...
				if (_weapon isKindOf 'StaticMortar') then {
					private _array = [];
					private _mortarMaxMagCountPerType = [
						['8rnd_82mm_mo_flare_white'	,	'8rnd_82mm_mo_smoke_white'	,	'8rnd_82mm_mo_shells'],
						[1 /*max is 1*/				,	1 /*max is 1*/				,	4 /*max is 4*/] /*-1 is same as max*/
					];
					/*
					...
					*/
					{
						...
						private _index = (_mortarMaxMagCountPerType # 0) find (tolower _magazineType);
						if!(_index < 0) then {
							private _max = _mortarMaxMagCountPerType # 1 # _index;
							if (_max == 0) then {
								_array pushBack [_magazineType,_turret];
							} else {
								(_mortarMaxMagCountPerType # 1) set [_index, (_max-1)];
							};
						};
					} forEach (magazinesAllTurrets _weapon);
