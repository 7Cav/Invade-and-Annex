...
if (((count allPlayers) < 15) && (_playerJetCount isEqualTo 0)) exitWith {};

_jetSelect = selectRandomWeighted [
	'O_Plane_CAS_02_dynamicLoadout_F',0.3,
	'O_Plane_Fighter_02_F',([0,0.025] select (_playerJetCount > 0)),
	'O_Plane_Fighter_02_Stealth_F',([0,0.025] select (_playerJetCount > 0)),
	'I_Plane_Fighter_03_AA_F',([0,0.3] select (_playerJetCount > 0)),
	'I_Plane_Fighter_03_dynamicLoadout_F',0.3,
	'I_Plane_Fighter_04_F',([0,0.3] select (_playerJetCount > 0)),
	'c_plane_civil_01_racing_f',0.05,
...
// Replace AMRAAMs and AR73s (Code by McDodelijk)
{
// Replacement Options:
// Flachion-22: "PylonMissile_1Rnd_Missile_AA_04_F", "PylonRack_1Rnd_Missile_AA_04_F"
// Zephyr: "PylonMissile_1Rnd_GAA_missiles", "PylonRack_1Rnd_GAA_missiles"
// BIM 9X: "PylonMissile_Missile_BIM9X_x1", "PylonRack_Missile_BIM9X_x1", "PylonRack_Missile_BIM9X_x2"
// AA 73: "PylonMissile_Missile_AA_R73_x1"
// ASRAAM: "PylonMissile_1Rnd_AAA_missiles", "PylonRack_1Rnd_AAA_missiles"

    private _pylonmag = _x;
    switch true do {
        case (_pylonmag in ["PylonMissile_Missile_AA_R77_x1", "PylonMissile_Missile_AA_R77_INT_x1"]): {
            _jetActual setPylonLoadOut [_forEachIndex+1, "PylonMissile_Missile_AA_R73_x1", true];
        };
        case (_pylonmag in ["PylonMissile_Missile_AMRAAM_D_x1", "PylonMissile_Missile_AMRAAM_D_INT_x1", "PylonMissile_Missile_AMRAAM_C_x1"]): {
            _jetActual setPylonLoadOut [_forEachIndex+1, "PylonMissile_1Rnd_Missile_AA_04_F", true];
        };
        case (_pylonmag in ["PylonRack_Missile_AMRAAM_C_x1", "PylonRack_Missile_AMRAAM_D_x1"]): {
            _jetActual setPylonLoadOut [_forEachIndex+1, "PylonRack_1Rnd_GAA_missiles", true];
        };
        case (_pylonmag in ["PylonRack_Missile_AMRAAM_C_x2", "PylonRack_Missile_AMRAAM_D_x2"]): {
            _jetActual setPylonLoadOut [_forEachIndex+1, "PylonRack_Missile_BIM9X_x2", true];
        };
    }
} foreach (getPylonMagazines _jetActual);

// Next foreach is not necessary
{
    if(_x in ["weapon_AMRAAMLauncher", "weapon_R77Launcher"]) then {
        _jetActual removeWeapon _x;
    };
} foreach (weapons _jetActual);
