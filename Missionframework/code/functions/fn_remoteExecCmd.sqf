...
if (_type isEqualTo 'setVehicleAmmoDef') exitWith {
	_1 setVehicleAmmo _2;
};
...
if !(remoteExecutedOwner == 0) then {
	[(str _this),
	{
		format ["WARNING: %1(UID: %2) initialized fn_remoteExecCmd with an unsupported command: %3", profileName, getPlayerUID player, _this] remoteExec ["diag_log", 2];
	}] remoteExec ["call", remoteExecutedOwner];
} else {
	format ["WARNING: %1(UID: %2) initialized fn_remoteExecCmd with an unsupported command: %3", profileName, getPlayerUID player, str _this] remoteExec ["diag_log", 2];
};