...
private [
	...
	'_QS_v','_QS_arr','_QS_BADGUY','_QS_fuzzyPos','_QS_medicalVehicles','_QS_missionComplete','_QS_missionFailed','_QS_civilianNeutrality',
...
if (worldName isEqualTo 'Tanoa') then {
	_QS_locationsUrban = [
		...
		/*/[[1804.05,11985.5,0.00139904],""],/*/ //Too cloase to base
...
[0,"Rescue P.O.W.",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
...
for '_x' from 0 to 1 step 0 do {
	...
	if (_QS_missionFailed) exitWith {
		...
		[2,"The P.O.W. has died",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if (_QS_missionComplete) exitWith {
		...
		[1,"The P.O.W. has been rescued",0,70] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if (!(_QS_enemyDetected)) then {
		...
		if (_QS_enemyDetected) then {
			...
			[4,"POW Mission: You got detected",0,0] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if (!(_QS_powRescued)) then {
		if (missionNamespace getVariable 'QS_sideMission_POW_rescued') then {
			...
			[4,"Extract P.O.W.",0,70] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	if ((missionNamespace getVariable 'QS_sideMission_civsKilled') > 0) then {
		if (!(_QS_civiliansUnhappy)) then {
			...
			[4,"The civilians are now unhappy",0,70] remoteExec ['NL_fnc_objPopup', -2, FALSE];
