...
private _c = FALSE;
if !(localNamespace getVariable ["McD_afk_lastkeypressed", -1] isEqualTo _key) then {
	player setVariable ['QS_client_afkTimeout',time,FALSE];
	localNamespace setVariable ["McD_afk_lastkeypressed", _key];
};

if (_key isEqualTo 0x0B) then {
...
if (_key isEqualTo 0x19) then {
    jsocHUD = profileNamespace getVariable ["jsocHUD", true];
    if (jsocHUD isEqualTo false) then {
        showHUD [true,true,true,true,true,true,true,true,true,true];
        profileNamespace setVariable ["jsocHUD", true];
    } else {
        showHUD [false,false,false,false,false,false,false,false,false,false];
        profileNamespace setVariable ["jsocHUD", false];
    };
    _c = TRUE;
};
