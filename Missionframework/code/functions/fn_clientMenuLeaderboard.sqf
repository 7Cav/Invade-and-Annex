...
_list = [
	...
	[3,'Dog tags','Dog tag leaderboard'],
	[4,'Officer Barrets','barret leaderboard'],
...
if (_type isEqualTo 'onLoad') exitWith {
	...
	(_display displayCtrl 1803) ctrlSetText 'jsoc\patreon\insignia\comm_patch.paa';
				...
				lnbSetColumnsPos [1808,[(1 * (safezoneW * 0.03)),(4 * (safezoneW * 0.03)),(7 * (safezoneW * 0.03)),(10 * (safezoneW * 0.03))]];
				{
					...
					if (!((squadParams _object) isEqualTo [])) then {
						if (!((((squadParams _object) select 0) select 4) isEqualTo '')) then {
							...
							lnbSetPicture [1808,[_forEachIndex,3],'jsoc\patreon\insignia\comm_patch.paa'];
						};
					} else {
