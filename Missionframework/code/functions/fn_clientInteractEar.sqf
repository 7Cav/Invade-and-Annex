...
if ((!isNil {player getVariable 'QS_ears_collected'}) && (_obj in (player getVariable 'QS_ears_collected'))) exitWith {
	(missionNamespace getVariable 'QS_managed_hints') pushBack [5,TRUE,3,-1,'Already collected a Dog tag from this corpse!',[],-1];
};
...
playSound 'ClickSoft';
50 cutText ['Dog tag collected','PLAIN DOWN'];
TRUE;
