...
for '_x' from 0 to (1 + (round(random 2))) do {
	...
	//	if ((random 1) >= 0) then {
			[_box,1,nil] call (missionNamespace getVariable 'QS_fnc_customInventory');
	//	};
		if (!((toLower _boxSelect) in ['o_supplycrate_f','o_cargonet_01_ammo_f','box_east_ammoveh_f','i_e_cargonet_01_ammo_f','ig_supplycrate_f', 'i_supplycrate_f'])) then {
