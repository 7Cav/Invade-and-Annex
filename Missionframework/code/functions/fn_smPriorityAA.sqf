...
Last modified:
	...
	2020 A3 1.94 by Alx & McDodelijk
...
private _targetingRange_plane = [15000,11000] select (worldName in ['Tanoa','Malden']);
private _targetingRange_heli = [13000,11000] select (worldName in ['Tanoa','Malden']);
private _targetingRange_stealthCoef_heli = 0.7;
private _targetingRange_stealthCoef_plane = 0.7;
private _targetingAltitude_stealthCoef_heli = 1;
private _targetingAltitude_stealthCoef_plane = 1;
private _targetingAltitudeMin_heli = random [15,20,25];
private _targetingAltitudeMin_plane = random [25,30,35];
private _entitiesParams = [['Air'],['UAV_01_base_F','UAV_06_base_F','ParachuteBase'],FALSE,TRUE];
private _rearmInterval = _time + (60 + (random 60));
private _rearming = FALSE;
private _rearmDelay = [10,15,20];
...
for '_x' from 0 to 1 step 0 do {
	_spawnPosition = ['RADIUS',_basepos,5000,'LAND',[5,0,0.2,5,0,FALSE,objNull],TRUE,[],[],TRUE] call (missionNamespace getVariable 'QS_fnc_findRandomPos');
	if ((_spawnPosition distance2D _basepos) > 2600) then {
...
private _compositionData = [
	[
		["o_apc_tracked_02_aa_f",[0.230469,-6.17627,0.0173378],179.236,[],TRUE,TRUE,FALSE,{(_this select 0)}], 			// o_sam_system_04_f   O_APC_Tracked_02_AA_F
	...
	[
		...
		["O_T_APC_Tracked_02_AA_ghex_F",[-0.321777,8.54443,0.0163908],359.998,[],TRUE,TRUE,FALSE,{(_this select 0)}], 		// o_sam_system_04_f   O_T_APC_Tracked_02_AA_ghex_F
...
[
	...
	TRUE,
	[
		'The enemy has set up an Anti-Air Battery near our base. This is a priority target, soldiers! It has extremely long range! Get over there and take it out at all cost. While it is active, pilots will need to exercise caution when flying. This objective is not accurately marked. While it is re-arming, it will not be able to lock on at long range. Bringing heavy explosives is advised.',
		'Anti-Air Battery',
		'Anti-Air Battery'
	],
...
[5,"Anti-Air Battery",0,20] remoteExec ['NL_fnc_objPopup', -2, FALSE];
//comment 'Loop';
...
for '_x' from 0 to 1 step 0 do {
	if (((_aaHulls findIf {(alive _x)}) isEqualTo -1) || {(missionNamespace getVariable ['QS_smSuccess',FALSE])}) exitWith {
		...
		[1,"Anti-Air Battery Neutralized",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
		[40] call NL_fnc_addScore;
	...
	if (_time > _checkDelay) then {
		...
		_targetListEnemy = (entities _entitiesParams) select {private _targetCandidate = _x; ((side _targetCandidate) isEqualTo WEST) && {!(((crew _targetCandidate) findIf {(alive _x)}) isEqualTo -1)}};
		_targetListEnemy = _targetListEnemy select {
			private _targetCandidate = _x;
			private _targetType = toLower (typeOf _targetCandidate);
			private _isStealth = (_targetType in _stealthAircraft);
			private _isLaserOff = (!(isLaserOn _targetCandidate));
			private _isRadarOff = (!(isVehicleRadarOn _targetCandidate));
			private _distance = (_targetCandidate distance2D _spawnPosition);
			private _aircraftPosition = getPosATL _targetCandidate;
			if (surfaceIsWater _aircraftPosition) then { _aircraftPosition = ATLToASL _aircraftPosition; };
			private _altitude = _aircraftPosition # 2;

			private _isTarget = if (_targetCandidate isKindOf 'Plane') then {
				if (_isStealth && _isLaserOff && _isRadarOff) then {
					_distance < (_targetingRange_plane * _targetingRange_stealthCoef_plane)
					&&
					{_altitude > (_targetingAltitudeMin_plane * _targetingAltitude_stealthCoef_plane)}
				} else {
					_distance < _targetingRange_plane
					&&
					{_altitude > _targetingAltitudeMin_plane}
				};
			} else {
				if (_targetCandidate isKindOf 'Helicopter') then {
					if (_isStealth && _isLaserOff && _isRadarOff) then {
						_distance < (_targetingRange_heli * _targetingRange_stealthCoef_heli)
						&&
						{_altitude > (_targetingAltitudeMin_heli * _targetingAltitude_stealthCoef_heli)}
					} else {
						_distance < _targetingRange_heli
						&&
						{_altitude > _targetingAltitudeMin_heli}
					};
				} else {
					FALSE
				};
