...
if (_type isEqualTo 'onLoad') then {
	...
	(_display displayCtrl 1807) ctrlSetTooltip (format ["0.1 - 1.1 (vanilla default 1, server default and minimum %1)", (missionNamespace getVariable ['QS_missionConfig_minAimCoef',0.1])]);
...
if (_type isEqualTo 'AimCoefSlider') then {
	...
	_value = (_value - ((_value*10) % (0.1*10))/10); // times 10 is needed because of floating point numbers inaccuracy
	_value = _value max (missionNamespace getVariable ['QS_missionConfig_minAimCoef',0.1]);
	player setCustomAimCoef _value;
	player setVariable ['QS_stamina',[(isStaminaEnabled player),_value],FALSE];
	(_this # 1) sliderSetPosition _value;
	ctrlSetText [1808,format ['%1',_value]];
};
