...
_vehicle setVariable ['McD_AO_AA',TRUE,TRUE];
...
[0,_vehicle,EAST,1] call (missionNamespace getVariable 'QS_fnc_vSetup2');
{
	...
	[
		'Killed',
		{
			...
			if ((missionNamespace getVariable 'QS_mission_aoType') isEqualTo 'SC') then {
				...
				[1,"AA Site Destroyed",0,10] remoteExec ['NL_fnc_objPopup', -2, FALSE];
	...
	[
		'Deleted',
		{
			...
			_vehicleMarkers = _entity getVariable ['QS_vehicle_markers',[]];
