...
if (_type isEqualTo 0) exitWith {
	...
	_subObjectiveData = missionNamespace getVariable ['QS_classic_subObjectiveData',[]];
	{
		...
		if (_objectiveState isEqualTo 0 || _subType in ['ENEMYPOP', 'AA', 'SUBOBJECTIVES', 'SMALLTASKS']) then {
...
if (_type isEqualTo 1) exitWith {
	...
	if (_subType isEqualTo 'JAMMER') then {
		...
		_roughPos = [((_position select 0) - 140) + (random 280),((_position select 1) - 140) + (random 280),0];
		_jammer = [1,'QS_ao_jammer_1',_position,_roughPos,(ceil (SelectRandom [450,500,550,600,650,700,750,800,900]))] call (missionNamespace getVariable 'QS_fnc_gpsJammer');
	...
	if (_subType isEqualTo 'AA') then {
		...
		_objectiveCode = {
			...
			_pos = missionNamespace getVariable ['QS_aoPos',[0,0,0]];
			_aoSize = missionNamespace getVariable ['QS_aoSize',500];
			if (count ((vehicles select {(alive _x) && (_x getVariable ['McD_AO_AA',FALSE])}) inAreaArray [_pos,_aoSize,_aoSize,0,FALSE,-1]) == 0) then {
				_return = 1;
			};
			_return;
		};
	...
	if (_subType isEqualTo 'SUBOBJECTIVES') then {
		_objectiveIsRequired = 1;
		_objectiveArguments = [];
		_objectiveCode = {
			//params [''];
			private _return = 1;
			{
				if !(_x call (missionNamespace getVariable 'QS_fnc_scSubObjective')) then {
					_return = 0;
				};
	...
	if (_subType isEqualTo 'SMALLTASKS') then {
		_objectiveIsRequired = 1;
		_objectiveArguments = [];
		_objectiveCode = {
			private _return = if ((missionNamespace getVariable ["QS_module_aoSmallTasks_isActive", FALSE])) then {0} else {1};
			_return;
		};
		_objectiveOnCompleted = {};
		_objectiveOnFailed = {};
