...
if (_type isEqualTo 1) exitWith {
	private ['_text','_arsenalService','_respawnEnabled','_respawnTickets','_vehicleRespawnEnabled','_vehicleRepairService','_vehicleAmmoService','_radarServices','_vehicleFuelService','_personalRespawn'];
	_text = format [
	...
	*/
	if (!isNull (missionNamespace getVariable ['QS_module_fob_supplycrate', objNull]) && {!isObjectHidden (missionNamespace getVariable ['QS_module_fob_supplycrate', objNull])}) then {
		_arsenalService = '<t size="1" align="left">Arsenal</t><t color="#008000" size="1" align="right">Online</t><br/>';
	} else {
		_arsenalService = '<t size="1" align="left">Arsenal</t><t color="#ff0000" size="1" align="right">Offline</t><br/>';
	};
	_text = _text + _arsenalService;
...
if (_type isEqualTo 3) exitWith {
	if ((player getUnitTrait 'uavhacker') || (player getUnitTrait 'QS_trait_pilot') || (player getUnitTrait 'QS_trait_fighterPilot')) then {
		50 cutText ['Pilots/UAV cannot respawn at the FOB!','PLAIN'];
	} else {
