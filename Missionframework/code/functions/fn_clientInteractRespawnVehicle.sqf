...
if (!(((crew _t) findIf {(alive _x)}) isEqualTo -1)) exitWith {
	50 cutText ['Vehicle is not empty.','PLAIN',0.3];
};
...
// Extra case for the boat
if(_t getVariable ["NL_object_boatEngineer", false]) exitWith {
	if (alive _t) then {
		if (player canAdd "Toolkit") then {
			if(local _t) then {
				deleteVehicle _t;
			} else {
				[_t] remoteExecCall ["deleteVehicle", _t, false];
			};
			player addItem "ToolKit";
			50 cutText [(format ['%1 deflated, toolkit added',(getText (configFile >> 'CfgVehicles' >> (typeOf _t) >> 'displayName'))]),'PLAIN DOWN'];
		} else {
			2 cutText ["Not enough inventory space to pickup the boat","PLAIN",2];
		};
	} else {
		2 cutText ["The boat is too damaged to be recovered","PLAIN",2];
	};
};
