...
if (!isNull _objectParent) then {
	if (
		...
		if (
			(!alive _objectParent) ||
			{((driver _objectParent) == _unit) && (_objectParent isKindOf "LandVehicle" || {_objectParent isKindOf "Ship"})}
		) then {
			[_unit, _objectParent] call (missionNamespace getVariable 'McD_fnc_incapacitatedEject');
			/*
			private _vel = velocity _objectParent;
			private _engine = isEngineOn _objectParent;
			moveOut _unit;
			unassignVehicle _unit;
			waitUntil {(isNull (objectParent _unit))};
			uiSleep 0.01;
			private _ejectDirection = if(!alive _objectParent) then {random 360} else {_objectParent getDir _unit};
			private _ejectSpeed = if(!alive _objectParent) then {(3 + random 12)} else {3};
			private _ejectVelocityVertical = if(!alive _objectParent) then {(2 + random 8)} else {2};
			_unit setDir _ejectDirection;
			_unit setVelocity [
				(_vel select 0) + ((sin (_objectParent getDir _unit)) * _ejectSpeed),
				(_vel select 1) + ((cos (_objectParent getDir _unit)) * _ejectSpeed),
				(_vel select 2) + _ejectVelocityVertical
			];
			if(alive _objectParent) then {
				_objectParent engineOn _engine;
				//_objectParent setVelocity _vel; // Does not work, due to missing driver causing the emergency breaking
			};
			*/
			// requiered before setting unit incapacitated
			uiSleep 0.1;
		} else {
...
private _medicalTimer = _medicalStartTime + _medicalTimerDelay;
{
	...
	['QS_respawn_disable',(diag_tickTime + 60),FALSE],
...
*/
private _actOfGod = TRUE;
private _fnc_chance = {
	if!(player getUnitTrait 'medic') exitWith { 0 };
	private _c = 0.35;
	{ _c = _c + (((_x # 1)/500) ^ 2) - 1; } foreach ([player, 500] call McD_fnc_getNearbyMedicDetail);
	_c;
};
private _actOfGod_delay = _tickTimeNow + 25 + (random 25);
...
private _buttonRespawnBase = controlNull;
private _buttonRespawnFOB = controlNull;
private _QS_buttonMedevac = controlNull;
private _QS_buttonAction = '';
private _playerClassDName = (['GET_ROLE_DISPLAYNAME',(_unit getVariable ['QS_unit_role','rifleman'])] call (missionNamespace getVariable ['QS_fnc_roles',{'rifleman'}]));
...
private _text1 = parseText format ['<t size="1.5" align="left">INCAPACITATED<t/><br/><t size="1" align="left">Press [%1] to respawn<br/>Bleeding out (%2)<br/>%3</t>',(_string1 select [1,((count _string1) - 2)]),([(_medicalTimer - _tickTimeNow),'MM:SS'] call (missionNamespace getVariable 'BIS_fnc_secondsToString')),([player, 500] call (missionNamespace getVariable 'QS_fnc_clientMFindHealer'))];
...
*/
TRUE call (missionNamespace getVariable 'McD_fnc_incapacitatedVisualEffects');
...
private _isPilot = ((_unit getUnitTrait 'QS_trait_pilot') || {(_unit getUnitTrait 'QS_trait_fighterPilot')});
private _towerCheckDelay = _tickTimeNow - 1;
private _towerCheckDelayFixed = 15;
...
_fn_towerTeleport = missionNamespace getVariable 'McD_fnc_incapacitatedTowerTeleport';
private _fnc_nearMedicDistance = {
	100 max (500 min (600 - floor (((diag_tickTime - _medicalStartTime) / _medicalTimerDelay) * 600)));
};
//comment 'Loop';
for '_x' from 0 to 1 step 0 do {
	...
	_text1 = parseText format ['<t size="1.5" align="left">INCAPACITATED<t/><br/><t size="1" align="left">Press [%1] to respawn<br/>Bleeding out (%2)<br/>%3</t>',(_string1 select [1,((count _string1) - 2)]),([(_medicalTimer - _tickTimeNow),'MM:SS'] call _fn_secondsToString),([player, 0 call _fnc_nearMedicDistance] call _fn_findHealer)];
	_ctrlIncapacitated ctrlSetStructuredText ([_text1,(parseText '')] select visibleMap);
	// Tower teleporter
	if (_tickTimeNow > _towerCheckDelay) then {
		0 call _fn_towerTeleport;
		_towerCheckDelay = _tickTimeNow + _towerCheckDelayFixed;
	};
	...
	if (!((_unit getVariable ['QS_revive_respawnType','']) isEqualTo '')) then {
		// checking if there are any medics nearby. If so, prevent respawn
		private _nearMedics = [_unit, 0 call _fnc_nearMedicDistance] call McD_fnc_getNearbyMedicDetail;
		_unit setVariable ["McD_RespawnWatch_alive", FALSE, FALSE];
		_unit setVariable ["McD_RespawnWatch_RespawnPosition", position _unit, FALSE];
		if (count _nearMedics == 0) then {
			...
			// Exception for pilots when other pilots are down.
			if (_isPilot) then {
				private _pilots = allPlayers select {(_x getUnitTrait 'QS_trait_pilot')};
				if ({lifeState _x in ["HEALTHY", "INJURED"]} count _pilots == 0) then {
					_unit setVariable ['QS_revive_respawnType','BASE',FALSE];
					_forceRespawned = TRUE;
				};
			};
			if !(_forceRespawned) then {
				_unit setVariable ['QS_revive_respawnType','',FALSE];
				titleText ["Don't give up yet. There are medics nearby!", "PLAIN"];
			};
		};
	};
	//comment 'Revived';
	...
	if (_tickTimeNow > _ambulanceDelay) then {
		if (((_unit distance2D _medevacBase) < 10) || {([0,_unit] call _fn_isNearFieldHospital)}) then {
			if (isNull _objectParent) then {
				if (isNull _attachedTo) then {
	...
	if (!isNull _objectParent) then {
		if (
			!alive _objectParent || // Vehicle did explode
			(_unit == driver _objectParent && (driver _objectParent) != (currentPilot _objectParent)) // I'm the pilot, but copilot took over
		) then {
			/*
			_unit setUnconscious FALSE;
			uiSleep 0.1;
	...
	if (!(_49Opened)) then {
		...
		if (!isNull _d49) then {
			...
			_buttonRespawnBase = _d49 displayCtrl 103;
			_buttonRespawnBase ctrlEnable FALSE;
			_buttonRespawnBase ctrlSetText 'Respawn at Base';
			_buttonRespawnBase ctrlEnable (_tickTimeNow > (_unit getVariable ['QS_respawn_disable',-1]));
			_buttonRespawnBase ctrlSetTooltip 'Respawn at the main base.';
			_QS_buttonAction = "player setVariable ['QS_revive_respawnType','BASE',FALSE];";
			_buttonRespawnBase buttonSetAction _QS_buttonAction;
			_buttonRespawnBase ctrlCommit 0;
			_buttonRespawnFOB = _d49 displayCtrl 1010;
			_QS_buttonAction = "player setVariable ['QS_revive_respawnType','FOB',FALSE];";
			_buttonRespawnFOB buttonSetAction _QS_buttonAction;
			...
			*/
			private _QS_buttonAbort = (_d49 displayCtrl 104);
			_QS_buttonAbort ctrlEnable TRUE;
			_QS_buttonAbort ctrlSetText 'Leave the server';
			_QS_buttonAbort ctrlSetTooltip 'This will kick you from the server to leave.';
			_QS_buttonAbort ctrlRemoveAllEventHandlers 'OnButtonClick';
			_QS_buttonAbort ctrlRemoveAllEventHandlers 'OnButtonDown';
			_QS_buttonAbort ctrlAddEventHandler ["ButtonClick", {
				[clientOwner, {
					params ["_cid"];
					if (remoteExecutedOwner isEqualTo _cid) then {
						([] call (uiNamespace getVariable 'QS_fnc_serverCommandPassword')) serverCommand (format ['#kick %1 Incapacitated player leaving the server',_cid]);
					};
		...
		if (isNull _d49) then {
			...
			if !(_tickTimeNow > (_unit getVariable ['QS_respawn_disable',-1])) then {
				_timeStr = ([((_unit getVariable ['QS_respawn_disable',-1]) - _tickTimeNow),'MM:SS'] call _fn_secondsToString);

				_buttonRespawnBase ctrlEnable FALSE;
				_buttonRespawnBase ctrlSetText _timeStr;
				_buttonRespawnBase ctrlCommit 0;

				_QS_buttonMedevac ctrlEnable FALSE;
				_QS_buttonMedevac ctrlSetText _timeStr;
				_QS_buttonMedevac ctrlCommit 0;

				_buttonRespawnFOB ctrlSetText _timeStr;
				_buttonRespawnFOB ctrlEnable FALSE;
				_buttonRespawnFOB ctrlCommit 0;
			} else { // activate buttons, depending on other conditions
				if (player getVariable ['QS_module_fob_client_respawnEnabled',TRUE]) then {
					if (missionNamespace getVariable 'QS_module_fob_respawnEnabled') then {
						if ((missionNamespace getVariable 'QS_module_fob_respawnTickets') > 0) then {
							if (_timeNow > (missionNamespace getVariable 'QS_module_fob_client_timeLastRespawn')) then {
...
*/
FALSE spawn (missionNamespace getVariable 'McD_fnc_incapacitatedVisualEffects');
