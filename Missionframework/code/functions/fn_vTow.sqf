...
_disAllowed = ["b_mbt_01_arty_f","b_t_mbt_01_arty_f","b_mbt_01_mlrs_f","b_t_mbt_01_mlrs_f","b_apc_tracked_01_aa_f","b_t_apc_tracked_01_aa_f"];
_towVs = [
...
if (({if(_x isEqualType objNull) then {FALSE} else {if(_x isEqualTo "") then {FALSE} else {TRUE};};} count (UAVControl _found)) > 0) exitWith {50 cutText [format ['Cannot tow %1: It is currently actively controlled by a UAV operator.',_foundName],'PLAIN DOWN',0.5];};
