				...
				if ((animationState _unit) in ['ainjppnemrunsnonwnondb_grab','ainjppnemrunsnonwnondb_still','acts_injuredlyingrifle02','ainjppnemstpsnonwnondnon']) then {
					...
					if ((animationState _unit) in [
						...
						player spawn {
								...
								[_this, 1] remoteExecCall ["setAnimSpeedCoef", 0, FALSE];
							};
						[player, 1.5] remoteExecCall ["setAnimSpeedCoef", 0, FALSE];
				...
				if (_unit getVariable ['QS_RD_escorted',FALSE]) then {
					...
					if (!(_unit isEqualTo (missionNamespace getVariable 'QS_sideMission_POW'))) then {
						if ((player distance2D (missionNamespace getVariable ['QS_prisonPos',(markerPos 'QS_marker_gitmo')])) < 20) then {
							...
							if (_unit isEqualTo (missionNamespace getVariable 'QS_csatCommander')) then {
								[1,"Captured commander imprisoned",0,40] remoteExec ['NL_fnc_objPopup', -2, FALSE];
								[40] call NL_fnc_addScore;
								missionNamespace setVariable ['QS_forceDefend', -1, TRUE];
								missionNamespace setVariable ['QS_csatCommander', objNull, TRUE];
							};
						};
					};
				};
