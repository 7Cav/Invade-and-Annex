...
Description:
	...
	Adds extra functionality to the default
	bis_fnc_dynamicGroups:
	- max squadsize 16 by default
	- private squads by defauöt
	- invite lifetime limit
	- accept only one invite per user action
	- keep track of squadlead even if he is down
__________________________________________________/*/
// Include common defines file
#include "\A3\Functions_F_MP_Mark\DynamicGroupsCommonDefines.inc"
#define VAR_GROUP_LEADER "mcd_dg_leader"
#define DG_DEFAULT_MAX_UNITS 16
#define DG_DEFAULT_PRIVATE TRUE
#define DG_DEFAULT_INVITELIFETIME 180
...
switch (_mode) do
{
	...
	case "Initialize" :
	{
		...
		private _maxUnitsPerGroup = _params param [1, DG_DEFAULT_MAX_UNITS, [0]];
	...
	case "OnClientMessage" :
	{
		...
		if (LOG_ENABLED) then
		{
			["OnClientMessage: Message (%1) received from client (%2 / %3) with data (%4) at serverTime (%5)", _variable, _player, name _player, _message, serverTime] call BIS_fnc_logFormat;
		};
	};
	...
	case "RegisterGroup" :
	{
		...
		if (!isNull _group && {!isNull _leader} && {_leader == leader _group}) then
		{
			...
			_private	= _data param [2, DG_DEFAULT_PRIVATE, [true]];
			...
			// Set the leader of this group
			_group setVariable [VAR_GROUP_LEADER, _leader, IS_PUBLIC];
	...
	case "UnregisterGroup" :
	{
		...
		if (!isNull _group && {["IsGroupRegistered", [_group]] call GROUPS}) then
		{
			if (_keep || {count units _group > 0}) then
			{
				...
				_group setVariable [VAR_GROUP_LEADER, nil, IS_PUBLIC];
			}
			else
			{
	...
	case "SwitchLeader" :
	{
		...
		{
			// Set new leader
			// if old leader is active leader or not part of the squad
			if (leader _group == (_group getVariable [VAR_GROUP_LEADER, objNull]) || !(_group getVariable [VAR_GROUP_LEADER, objNull] in units _group)) then {
				_group setVariable [VAR_GROUP_LEADER, _player, IS_PUBLIC];
			};
	...
	case "PlayerIsLeader" :
	{
		...
		if ((["PlayerHasGroup", [_player]] call GROUPS)) then {
			if ( _player == (leader group _player) ) then {
				// old leader is not part of the squad anymore - promote yourself
				if !( ((group _player) getVariable [VAR_GROUP_LEADER, objNull]) in (units group _player) ) then {
					["SendClientMessage", ["SwitchLeader", [group player, player]]] call GROUPS;
				};
				TRUE
			} else {
				if ( _player == ((group _player) getVariable [VAR_GROUP_LEADER, objNull]) ) then {
					TRUE
				} else {
					FALSE
				};
	...
	case "OnKeyUp" :
	{
		...
		uiNamespace setVariable ["BIS_dynamicGroups_ignoreInviteAccept", false];
	...
	case "UpdateKeyDown" :
	{
		...
		if (!isNil { uiNamespace getVariable "BIS_dynamicGroups_keyDownTime" } && {count (["GetPlayerInvites", [player]] call GROUPS) > 0}) then
		{
			...
			if (_timeHolding >= HOLD_DOWN_TIME_FOR_INVITE_ACCEPT && !(uiNamespace getVariable ["BIS_dynamicGroups_ignoreInviteAccept", false])) then
			{
				uiNamespace setVariable ["BIS_dynamicGroups_ignoreInviteAccept", true];
				private "_invites";
	...
	case "AddInvite" :
	{
		...
		if !(["PlayerIsLeader", [_from]] call GROUPS) exitWith {
			// Do not allow invites from not squadleads
		};
		...
		if (_index != -1) then
		{
			_invitations set [_index, [_group, _from, _to, serverTime]];
		}
		else
		{
			_invitations pushBack [_group, _from, _to, serverTime];
		};
	...
	case "HasInvite" :
	{
...

		{
			...
			if (_group == _inviteGroup && {_player == _inviteTo} && {serverTime <= _inviteTime + INVITE_LIFETIME}) exitWith
			{
				_hasInvitation = true;
			};
		} forEach _invitations;
	...
	case "GetPlayerInvites" :
	{
		...
		_player         = _params param [0, objNull, [objNull]];
		_maxLifeTime    = _params param [1, DG_DEFAULT_INVITELIFETIME, [0]];
		...
		{
			if (!isNull (_x select 0) && {serverTime - (_x select 3) < _maxLifeTime}) then
			{
